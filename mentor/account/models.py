...
class Account(SurrogatePK, Model):
    ...
    account_profile = relationship('Accountprofile', uselist=False, back_populates='account', cascade='all, delete-orphan',lazy='select')
...
class Accountprofile(SurrogatePK, Model):
    ...
    marital_status = Column(db.Enum('single', 'married', 'divorced', 'widowed', name='varchar'))
    education = Column(db.Enum('undergraduate', 'graduate', 'post_graduate', name='varchar'))
    account_id = Column(db.Integer, ForeignKey('account.id', ondelete='CASCADE'), unique=True, index=True)

    account = relationship('Account', back_populates="account_profile", single_parent=True)
...
class UserRoles(SurrogatePK, Model):
    ...
    def __repr__(self):
        """Represent instance as a unique string."""
        return '<UserRoles({account_id!r}, {role_id!r})>'.format(account_id=self.account_id, role_id=self.role_id)