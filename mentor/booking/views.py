...
from .models import Booking, Tag, Slot, Booking_Tag, WeekDay
...
def get_slots():
    try:
        slot_data = Slot.query.order_by(Slot.created_at).all()
        return slot_data
    except Exception as e:
        return {"message": str(e)}, 400
...
@blueprint.route("/testingroute", methods=["GET"])
def create_test():
...
def create_weekday(name):
    try:
        ...
        return Response(
            json.dumps({"response": weekday.name + " successfully created"}), status=201, mimetype="application/json"
        )
    except Exception as e:
        return {"message": str(e)}, 400
...
def get_weekdays():
    try:
        weekday_data = WeekDay.query.order_by(WeekDay.created_at).all()
        return weekday_data
    except Exception as e:
        return {"message": str(e)}, 400
